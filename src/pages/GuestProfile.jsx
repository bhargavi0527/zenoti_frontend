  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        // Try to fetch by ID first, then by guest code if that fails
        let res = await fetch(`http://127.0.0.1:8000/guests/${encodeURIComponent(guestId)}`, {
          headers: { accept: 'application/json' }
        });
        
        // If ID fetch fails, try to fetch by guest code
        if (!res.ok && guestId.startsWith('GUEST')) {
          res = await fetch(`http://127.0.0.1:8000/guests/code/${encodeURIComponent(guestId)}`, {
            headers: { accept: 'application/json' }
          });
        }
        if (!res.ok) {
