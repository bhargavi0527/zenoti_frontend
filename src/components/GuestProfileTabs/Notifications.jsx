...
export default function NotificationsTab({ guestId }) {
  const [loading, setLoading] = useState(false);
  const [rows, setRows] = useState([]);
  const headers = ['Date', 'Channel', 'Subject', 'Status'];
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        setLoading(true);
        const res = await fetch(`http://127.0.0.1:8000/guests/${encodeURIComponent(guestId)}/notifications/`, { headers: { accept: 'application/json' } });
  ...
  const toDate = (d) => { try { return new Date(d).toLocaleString(); } catch { return ''; } };
